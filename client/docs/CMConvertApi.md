...
[**convertToWebP**](CMConvertApi.md#converttowebp) | **POST** /image/convert/to/webp | Convert input image to WebP format
...
// Convert input image to Bitmap BMP format
[apiInstance convertToBmpWithImageFile:imageFile
          completionHandler: ^(NSData* output, NSError* error) {
                        if (output) {
                            NSLog(@"%@", output);
                        }
                        if (error) {
                            NSLog(@"Error calling CMConvertApi->convertToBmp: %@", error);
                        }
                    }];
```
...
// Convert input image to GIF format
[apiInstance convertToGifWithImageFile:imageFile
          completionHandler: ^(NSData* output, NSError* error) {
                        if (output) {
                            NSLog(@"%@", output);
                        }
                        if (error) {
                            NSLog(@"Error calling CMConvertApi->convertToGif: %@", error);
                        }
                    }];
```
...
NSNumber* quality = @56; // Set the JPEG quality level; lowest quality is 1 (highest compression), highest quality (lowest compression) is 100; recommended value is 75
NSURL* imageFile = [NSURL fileURLWithPath:@"/path/to/file.txt"]; // Image file to perform the operation on.  Common file formats such as PNG, JPEG are supported.
...
[apiInstance convertToJpgWithQuality:quality
          ...
          completionHandler: ^(NSData* output, NSError* error) {
                        ...
                        if (error) {
                            NSLog(@"Error calling CMConvertApi->convertToJpg: %@", error);
                        }
                    }];
```
...
Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **quality** | **NSNumber***| Set the JPEG quality level; lowest quality is 1 (highest compression), highest quality (lowest compression) is 100; recommended value is 75 | 
 **imageFile** | **NSURL***| Image file to perform the operation on.  Common file formats such as PNG, JPEG are supported. | 
...
// Convert input image to Photoshop PSD format
[apiInstance convertToPhotoshopWithImageFile:imageFile
          completionHandler: ^(NSData* output, NSError* error) {
                        if (output) {
                            NSLog(@"%@", output);
                        }
                        if (error) {
                            NSLog(@"Error calling CMConvertApi->convertToPhotoshop: %@", error);
                        }
                    }];
```
...
// Convert input image to PNG format
[apiInstance convertToPngWithImageFile:imageFile
          completionHandler: ^(NSData* output, NSError* error) {
                        if (output) {
                            NSLog(@"%@", output);
                        }
                        if (error) {
                            NSLog(@"Error calling CMConvertApi->convertToPng: %@", error);
                        }
                    }];
```
...
// Convert input image to TIFF format
[apiInstance convertToTiffWithImageFile:imageFile
          completionHandler: ^(NSData* output, NSError* error) {
                        if (output) {
                            NSLog(@"%@", output);
                        }
                        if (error) {
                            NSLog(@"Error calling CMConvertApi->convertToTiff: %@", error);
                        }
                    }];
```
...
// Convert input image to WebP format
[apiInstance convertToWebPWithImageFile:imageFile
          completionHandler: ^(NSData* output, NSError* error) {
                        if (output) {
                            NSLog(@"%@", output);
                        }
                        if (error) {
                            NSLog(@"Error calling CMConvertApi->convertToWebP: %@", error);
                        }
                    }];
```
